...
if (config.pagemock) {
  app.use(require('koa-mock')({datadir : path.join(rootdir, 'test', 'mocks')}));
}
...
var footer = config.customFooter ||
             fs.readFileSync(path.join(viewDir, 'footer.html'), 'utf8');
...
app.use(markdownMiddleware({
  ...
  indexName : '_readme',
  cache : true,
  render : function(content) { return renderMarkdown(content, false); },
}));
