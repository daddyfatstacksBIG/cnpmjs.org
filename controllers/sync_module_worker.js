...
var USER_AGENT = 'sync.cnpmjs.org/' + config.version + ' hostname/' +
                 os.hostname() + ' syncModel/' + config.syncModel +
                 ' syncInterval/' + config.syncInterval + ' syncConcurrency/' +
                 config.syncConcurrency + ' ' + urllib.USER_AGENT;
...
function SyncModuleWorker(options) {
  ...
  this.syncingNames = {};
  this.nameMap = {};
  this.names.forEach(function(name) { this.nameMap[name] = true; }.bind(this));
...
SyncModuleWorker.prototype.finish = function() {
  ...
  this.log(
      ...
      this.startName, this.type, this.successes.length, this.fails.length,
...
SyncModuleWorker.prototype.log = function() {
  var str = '[' + utility.YYYYMMDDHHmmss() + '] ' +
...
SyncModuleWorker.prototype._saveLog = function() {
  ...
  co(function*() { yield logService.append(that._logId, logstr); })
      .then(function() {
        that._loging = false;
        if (that._log) {
          that._saveLog();
      ...
      .catch(function(err) {
        ...
        if (this._isEnd) {
          logger.error('[SyncModuleWorker] skip to save %s logstr: %s',
                       that._logId, logstr);
        }
      });
};
...
SyncModuleWorker.prototype.start = function() {
  ...
  that.log(
      ...
      that.username, that.names[0], that.concurrency, that.noDep, that._publish,
...
SyncModuleWorker.prototype._doneOne = function*(concurrencyId, name, success) {
  ...
  if (cache) {
    ...
    cache.del(cacheKey).catch(err => { logger.error(err); });
  }

  this.log('----------------- Synced %s %s -------------------', name,
  ...
  defer.setImmediate(function*() { yield that.next(concurrencyId); });
};
...
SyncModuleWorker.prototype.syncUpstream = function*(name) {
  ...
  if (r.status !== 201 || !r.data.ok) {
    return this.log('sync upstream %s error, status: %s, response: %j', url,
                    r.status, r.data);
  }
  ...
  while (true) {
    ...
    if (rs.status !== 200 || !rs.data.ok) {
      this.log('sync upstream %s error, status: %s, response: %j', synclogURL,
               rs.status, rs.data);
      break;
    }
...
SyncModuleWorker.prototype.next = function*(concurrencyId) {
  ...
  // cnpmjs.org
  const registry = config.sourceNpmRegistryIsCNpm ? config.sourceNpmRegistry
...
SyncModuleWorker.prototype.syncByName = function*(concurrencyId, name, registry,
  ...
  if (common.isPrivateScopedPackage(name)) {
    this.log('[c#%d] [%s] ignore sync private scoped %j package', concurrencyId,
             name, config.scopes);
  ...
  try {
    ...
    if (registry === config.officialNpmReplicate) {
      try {
        const officialResult = yield npmSerivce.request(
            packageUrl, {registry : config.officialNpmRegistry});
        ...
        this.log(
            ...
            concurrencyId, name, officialPkg['dist-tags'],
            officialPkg.time && officialPkg.time.modified, pkg['dist-tags'],
            pkg.time && pkg.time.modified, packageUrl);
        if (officialPkg.time) {
          if (!pkg.time || officialPkg.time.modified > pkg.time.modified) {
            this.log(
                ...
                concurrencyId, name, pkg.time && pkg.time.modified,
        ...
        that.log('[c#%s] [error] [%s] get package(%s%s) error: %s',
                 concurrencyId, name, config.officialNpmReplicate, packageUrl,
                 err);
      }
    }
  } catch (err) {
    ...
    if (!err.res || err.res.statusCode !== 404) {
      ...
      this.log('[c#%d] [%s] retry from %s, retryCount: %s', concurrencyId, name,
               config.officialNpmRegistry, retryCount);
      try {
        var result = yield npmSerivce.request(
            packageUrl, {registry : config.officialNpmRegistry});
        pkg = result.data;
        status = result.status;
        realRegistry = config.officialNpmRegistry;
        ...
        that.log('[c#%s] [error] [%s] get package(%s%s) error: %s, status: %s',
                 concurrencyId, name, config.officialNpmRegistry, packageUrl,
                 errMessage, status);
        // sync fail
        yield that._doneOne(concurrencyId, name, false);
        return;
  ...
  if (status === 404 && pkg && pkg.reason &&
    ...
    try {
      var result = yield npmSerivce.request(
          packageUrl, {registry : config.sourceNpmRegistry});
      pkg = result.data;
      status = result.status;
      ...
      if (!err.res || err.res.statusCode !== 404) {
        var errMessage = err.name + ': ' + err.message;
        that.log('[c#%s] [error] [%s] get package(%s%s) error: %s, status: %s',
                 concurrencyId, name, config.sourceNpmRegistry, packageUrl,
                 errMessage, status);
        yield that._doneOne(concurrencyId, name, false);
        return;
      }
  ...
  if (status === 404) {
    ...
    if (name.length < 80 && pkg && pkg.time && pkg.time.unpublished &&
  ...
  that.log(
      ...
      concurrencyId, name, realRegistry, packageUrl, status, pkg['dist-tags'],
      pkg.time && pkg.time.modified, unpublishedInfo);

  if (unpublishedInfo) {
    try {
      yield that._unpublished(name, unpublishedInfo);
    } catch (err) {
      that.log('[c#%s] [error] [%s] sync error: %s', concurrencyId, name,
               err.stack);
      yield that._doneOne(concurrencyId, name, false);
      return;
    }
  ...
  this.log('[c#%d] [%s] synced success, %d versions: %s', concurrencyId, name,
...
function* _listStarUsers(modName) {
  ...
  var userMap = {};
  users.forEach(function(user) { userMap[user] = true; });
  return userMap;
}
...
SyncModuleWorker.prototype._unpublished = function*(name, unpublishedInfo) {
  ...
  this.log('    [%s] save unpublished info: %j to row#%s', name,
           unpublishedInfo, r.id);
  if (mods.length === 0) {
    return;
  }
  ...
  if (keys.length > 0) {
    try {
      ...
      // ignore error here
      this.log('    [%s] delete nfs files: %j error: %s: %s', name, keys,
...
SyncModuleWorker.prototype._sync = function*(name, pkg) {
  ...
  var result = yield [packageService.listModulesByName(name),
  ...
  if (config.enableAbbreviatedMetadata) {
    ...
    var packageUrl = '/' + name.replace('/', '%2f') +
                     '?cache=0&sync_timestamp=' + Date.now();
    ...
    if (result.status === 200) {
      ...
      try {
        ...
        that.log('  [%s] get abbreviated meta error: %s, headers: %j, %j', name,
                 err, result.headers, result.data);
      }
      if (data) {
  ...
  for (var i = 0; i < remoteVersionNames.length; i++) {
    ...
    if (exists.package && exists.package.dist.shasum === version.dist.shasum) {
      ...
      if (!existsModuleAbbreviated) {
        ...
        } else if (existsModuleAbbreviated.package.deprecated &&
                   !exists.package.deprecated) {
          // remove deprecated
          missingDeprecatedsOnExistsModuleAbbreviated.push({
            name,
      ...
      if ((version.publish_time === exists.publish_time) ||
        ...
        if (exists.description === null && version.description) {
          ...
          missingDescriptions.push(
              {id : exists.id, description : version.description});
          changedVersions[v] = 1;
        }
  ...
  for (var item of existsModuleAbbreviateds) {
    if (!remoteVersionNameMap[item.version] &&
  ...
  if (missingVersions.length === 0) {
    ...
    missingVersions.sort(function(
        a, b) { return a.publish_time - b.publish_time; });
    ...
    while (true) {
      try {
        ...
        that.log(
            ...
            syncModule.name, index, tries, delay, syncModule.version, err.name,
            err.stack);
    ...
    if (config.syncDeletedVersions) {
      ...
      if (downloadCount >= 10000) {
        ...
        if (deletedIn24HoursVersions.length > 0) {
          ...
          try {
            yield packageService.removeModulesByNameAndVersions(
                name, deletedIn24HoursVersions);
          } catch (err) {
            that.log('    [%s] delete error, %s: %s', name, err.name,
                     err.message);
          }
        }
        that.log(
  ...
  function* syncDes() {
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      var r = res[i];
      if (r.error) {
        that.log('    save error, id: %s, description: %s, error: %s', item.id,
                 item.description, r.error.message);
      } else {
        that.log('    saved, id: %s, description length: %d', item.id,
  ...
  function* syncTag() {
    if (deletedTags.length > 0) {
      ...
      that.log('  [%s] deleted %d tags: %j', name, deletedTags.length,
               deletedTags);
    }
    ...
    // sync tags
    var res = yield gather(missingTags.map(function(
        item) { return packageService.addModuleTag(name, item[0], item[1]); }));

    for (var i = 0; i < res.length; i++) {
      var item = missingTags[i];
      var r = res[i];
      if (r.error) {
        that.log('    add tag %s:%s error, error: %s', item.id,
                 item.description, r.error.message);
      } else {
        that.log('    added tag %s:%s, module_id: %s', item[0], item[1],
  ...
  function* syncModuleAbbreviateds() {
    ...
    that.log('  [%s] saving %d missing moduleAbbreviateds', name,
             missingModuleAbbreviateds.length);

    var res = yield gather(missingModuleAbbreviateds.map(function(
        item) { return packageService.saveModuleAbbreviated(item); }));

    for (var i = 0; i < res.length; i++) {
      var item = missingModuleAbbreviateds[i];
      var r = res[i];
      if (r.error) {
        that.log('    save moduleAbbreviateds error, module: %s@%s, error: %s',
                 item.name, item.version, r.error.message);
      } else {
        that.log('    saved moduleAbbreviateds, module: %s@%s', item.name,
                 item.version);
      }
    }
  }
  ...
  function* syncAbbreviatedMetadatas() {
    ...
    that.log('  [%s] saving %d abbreviated meta datas', name,
             missingAbbreviatedMetadatas.length);

    var res = yield gather(missingAbbreviatedMetadatas.map(function(
        item) { return packageService.updateModuleAbbreviatedPackage(item); }));
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      if (r.error) {
        ...
        that.log('    saved, module_abbreviated: %s@%s, %j', item.name,
                 item.version, item);
      }
    }
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      var r = res[i];
      if (r.error) {
        that.log('    save error, module id: %s, error: %s', item.id,
                 r.error.stack);
      } else {
  ...
  function* syncDeprecatedsOnExistsModuleAbbreviated() {
    ...
    that.log('  [%s] saving %d module abbreviated deprecated fields', name,
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      var r = res[i];
      if (r.error) {
        that.log('    save error, module abbreviated: %s@%s, error: %s',
                 item.name, item.version, r.error.message);
      } else {
        that.log('    saved, module abbreviated: %s@%s, deprecated: %j',
                 item.name, item.version, item.deprecated);
      }
    }
  }
  ...
  function* syncDeprecateds() {
    ...
    that.log('  [%s] saving %d Deprecated fields', name,
             missingDeprecateds.length);

    var res = yield gather(missingDeprecateds.map(function(item) {
      return packageService.updateModulePackageFields(
          item.id, {deprecated : item.deprecated});
    }));
  ...
  function* syncMissingUsers() {
    ...
    var map = {};
    rows.forEach(function(r) { map[r.name] = r; });
    ...
    that.log('  [%s] saving %d/%d missing npm users: %j', name,
             missingUsers.length, names.length, missingUsers);
    var res = yield gather(missingUsers.map(function(
        username) { return _saveNpmUser(username); }));
  ...
  function* syncMissingStarUsers() {
    ...
    var res = yield gather(missingStarUsers.map(function(
        username) { return packageService.addStar(name, username); }));
  ...
  function* syncNpmPackageMaintainers() {
    ...
    that.log('  [%s] syncing %d diff package maintainers: %j', name,
             diffNpmMaintainers.length, diffNpmMaintainers);
    var res = yield gather(diffNpmMaintainers.map(function(
        item) { return _saveMaintainer(name, item[0], item[1]); }));
  ...
  if (latestVersionPackageReadme.version && latestVersionPackageReadme.readme) {
    ...
    if (!existsPackageReadme ||
      ...
      var r = yield packageService.savePackageReadme(
          name, latestVersionPackageReadme.readme,
...
SyncModuleWorker.prototype._syncOneVersion = function*(versionIndex,
                                                       sourcePackage) {
  ...
  that.log(
      ...
      sourcePackage.name, versionIndex, delay, sourcePackage.version,
      sourcePackage.dist, that.noDep, that._publish, dependencies.length,
  ...
  try {
    // get tarball
    logger.syncInfo('[sync_module_worker] downloading %j to %j', downurl,
                    filepath);
    var r;
    try {
      r = yield urllib.request(downurl, downloadOptions);
    } catch (err) {
      logger.syncInfo('[sync_module_worker] download %j to %j error: %s',
                      downurl, filepath, err);
      throw err;
    }
    ...
    if (statusCode !== 200) {
      var err =
          new Error('Download ' + downurl + ' fail, status: ' + statusCode);
    ...
    logger.syncInfo('[sync_module_worker] uploaded, saving %j to database',
                    result);
    var r = yield afterUpload(result);
    logger.syncInfo('[sync_module_worker] sync %s@%s done!', sourcePackage.name,
  ...
  function* afterUpload(result) {
    ...
    if (config.enableAbbreviatedMetadata) {
      var moduleAbbreviatedResult =
          yield packageService.saveModuleAbbreviated(mod);
    ...
    that.log('    [%s:%s] done, insertId: %s, author: %s, version: %s, ' +
             ...
             sourcePackage.name, versionIndex, r.id, author, mod.version,
             dataSize, new Date(mod.publish_time), that._publish,
             moduleAbbreviatedId);
