...
describe('test/controllers/sync_module_worker.test.js', () => {
  ...
  it('should sync public scoped package', function*() {
    ...
    let envelope;
    mm(config, 'globalHook', function*(e) { envelope = e; });
  ...
  it('should start a sync worker with names and noDep', function(done) {
    var worker = new SyncModuleWorker(
        {name : [ 'mk2testmodule' ], noDep : true, username : 'fengmk2'});
  ...
  it('should start a sync worker with names', function(done) {
    var worker = new SyncModuleWorker(
        {name : [ 'mk2testmodule' ], username : 'fengmk2'});
  ...
  it('should sync unpublished info', function(done) {
    var worker = new SyncModuleWorker({name : [ 'afp' ], username : 'fengmk2'});
  ...
  it('should sync missing module abbreviateds deprecated property',
     function*() {
       var worker = new SyncModuleWorker({
       ...
       const rows = yield packageService.listModuleAbbreviatedsByName(
           'native-or-bluebird');
  ...
  describe('syncUpstream()', function() {
    it('should sync upstream work', function*() {
      var worker =
          new SyncModuleWorker({name : [ 'tnpm' ], username : 'fengmk2'});
      yield [worker.syncUpstream('tnpm'), worker.syncUpstream('pedding'), ];
    });
  });
  ...
  describe('sync deprecated info', () => {
    ...
    it('should sync support un-deprecate action', function*() {
      ...
      mm(packageService, 'listModulesByName', function*(name) {
        ...
        mods.forEach(function(
            mod) { mod.package.deprecated = 'mock deprecated'; });
        return mods;
      });
