...
describe('test/controllers/registry/package/list.test.js', () => {
  ...
  before(function(done) {
    ...
    var pkg = utils.getPackage('@cnpmtest/testmodule-list-1', '0.0.1',
                               utils.otherUser);
    ...
    request(app)
        ...
        .expect(201, function(err) {
          ...
          var pkg = utils.getPackage('@cnpmtest/testmodule-list-1', '1.0.0',
                                     utils.otherUser);
  ...
  it('should return all versions with cache-control', done => {
    mm(config, 'registryCacheControlHeader',
       'max-age=0, s-maxage=10, must-revalidate');
  ...
  it('should return all versions in abbreviated meta format for private scope package',
     function(done) {
       mm(config, 'registryCacheControlHeader',
          'max-age=0, s-maxage=10, must-revalidate');
       request(app)
  ...
  it('should show star users', function(done) {
    ...
    request(app)
        ...
        .expect(200, function(err, res) {
          ...
          var data = res.body;
          data.name.should.equal('@cnpmtest/testmodule-list-1');
          data.users.should.eql({fengmk2 : true, foouser : true});
          data.versions['0.0.1'].publish_time.should.equal(
              data.versions['0.0.1']._cnpm_publish_time);
          assert(/^W\/"\w{32}"$/.test(res.headers.etag));
          done();
        });
  });
  ...
  it('should support jsonp', function(done) {
    request(app)
        ...
        .expect(/jsonp\(\{/)
        .expect(res => { assert(/^W\/"\w{32}"$/.test(res.headers.etag)); })
        .expect(200, done);
  });
  ...
  it('should not sync not-exists package when config.syncByInstall = false',
     function(done) {
  ...
  it('should sync not-exists package when config.syncByInstall = true',
     function(done) {
       mm(config, 'syncByInstall', true);
       request(app).get('/should').expect(302, done);
     });
  ...
  describe.skip('unpublished', () => {
    ...
    it('should show unpublished info', done => {
      mm(config, 'syncModel', 'all');
      request(app).get('/moduletest1').expect(404, function(err, res) {
        should.not.exist(err);
        var data = res.body;
  ...
  describe('npm package', function() {
    before(function(done) { utils.sync('tair', done); });

    it('should show npm package after sync', function(done) {
      mm(config, 'syncModel', 'all');
      request(app).get('/tair').expect(200, function(err, res) {
        should.not.exist(err);
        var data = res.body;
         ...
         return request(app)
             ...
             .expect(res => {
               ...
               for (const v in data.versions) {
                 ...
                 assert(pkg.publish_time &&
                        typeof pkg.publish_time === 'number');
             ...
             .expect(res => {
               ...
               for (const v in data.versions) {
                 ...
                 assert(pkg.publish_time &&
                        typeof pkg.publish_time === 'number');
                 assert(pkg._publish_on_cnpm === undefined);
                 assert(pkg.dist.tarball.includes(
                     '.tgz?bucket=foo-us1&admin=1&other_urls=http'));
               }
               assert(/^W\/"\w{32}"$/.test(res.headers.etag));
             });
       });
