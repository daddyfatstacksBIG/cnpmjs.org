...
describe('test/controllers/registry/package/remove.test.js', function() {
  ...
  before(function(done) {
    var pkg = utils.getPackage('@cnpmtest/testmodule-remove-1', '1.0.0',
                               utils.otherUser);
    request(app)
        .put('/' + pkg.name)
  ...
  it('should remove all versions ok', function(done) {
    request(app)
        ...
        .expect(200, function(err) {
          ...
          request(app).get('/@cnpmtest/testmodule-remove-1').expect(404, done);
        });
  });
  ...
  it('should not remove nfs', function(done) {
    ...
    var pkg = utils.getPackage('@cnpmtest/testmodule-remove-2', '3.0.0',
                               utils.otherUser);
    request(app)
        .put('/' + pkg.name)
        .set('authorization', utils.otherUserAuth)
  ...
  describe('mock error', function() {
    beforeEach(function(done) {
      var pkg = utils.getPackage('@cnpmtest/testmodule-remove-mock-1', '2.0.0',
                                 utils.admin);
      request(app)
          .put('/' + pkg.name)
          .set('authorization', utils.adminAuth)
    ...
    it('should mock key not exists', function(done) {
      ...
      mm(packageService, 'listModulesByName', function*(name) {
        ...
        mods.forEach(function(mod) { delete mod.package.dist.key; });
        return mods;
      });
      request(app)
    ...
    it('should mock nfs remove error', function(done) {
      mm(nfs, 'remove',
         function*() { throw new Error('mock nfs remove error'); });
      request(app)
          .del('/@cnpmtest/testmodule-remove-mock-1/-rev/1')
          .set('authorization', utils.adminAuth)
          .expect(200, function(err) {
