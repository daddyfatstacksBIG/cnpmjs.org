...
exports.getTarballFilepath = function(filename) {
  ...
  var name = filename.replace(
      /\.tgz$/, '.' + crypto.randomBytes(16).toString('hex') + '.tgz');
  return path.join(config.uploadDir, name);
};

exports.getCDNKey = function(name, filename) {
  // if name is scope package name, need to auto fix filename as a scope package
  // file name e.g.: @scope/foo, filename: foo-1.0.0.tgz => filename:
  // @scope/foo-1.0.0.tgz
...
exports.setDownloadURL = function(pkg, ctx, host) {
  if (pkg.dist) {
    ...
    pkg.dist.tarball = util.format('%s://%s/%s/download/%s-%s.tgz', protocol,
                                   host, pkg.name, pkg.name, pkg.version);
    if (ctx.querystring) {
      var backupUrl = pkg.dist.tarball;
      pkg.dist.tarball += '?' + ctx.querystring +
                          '&other_urls=' + encodeURIComponent(backupUrl);
    }
  }
};

exports.isAdmin = function(
    username) { return typeof config.admins[username] === 'string'; };
...
exports.isMaintainer = function(user, maintainers) {
  ...
  var match =
      maintainers.filter(function(item) { return item.name === username; });
